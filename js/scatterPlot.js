...
export default class ScatterPlot {
  ...
  update(data, state) {
    ...
    this.x
      .domain([0, d3.max(data, (d) => d.populationMillions) * 1.08])
      .range([0, innerWidth])
      .nice();

    this.y
      .domain([0, d3.max(data, (d) => d.affectedMillions) * 1.1])
      .range([innerHeight, 0])
      .nice();

    const xAxis = d3.axisBottom(this.x).ticks(6).tickFormat(d3.format(",.0f"));
  ...
  getDimensions() {
    ...
    const width = Math.max(620, bounds.width || 620);
    const height = Math.max(460, bounds.height || 460);
